...
public class SelectedQuoteJsonSerializer {
    ...
    public void serialize(SelectedQuote selectedQuote, OutputStream os) {
        final JsonObjectBuilder job = Json.createObjectBuilder().add("author", selectedQuote.getQuote().getAuthor())
                .add("content", selectedQuote.getQuote().getContent()).add("range", selectedQuote.getCurrentIndex());
