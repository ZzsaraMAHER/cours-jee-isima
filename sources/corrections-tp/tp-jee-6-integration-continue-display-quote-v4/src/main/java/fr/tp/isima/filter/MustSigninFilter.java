...
public class MustSigninFilter implements Filter {
    ...
    private boolean isAllowedCall(final HttpServletRequest httpReq) {
        return isSigningRequest(httpReq) || isConnectedUser(httpReq) || isStaticRessource(httpReq) || isServiceRequest(httpReq);
    }

    private boolean isServiceRequest(HttpServletRequest httpReq) {
        return httpReq.getServletPath().startsWith("/service");
    }
